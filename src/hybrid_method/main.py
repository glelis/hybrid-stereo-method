...
def main(parameters):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(os.path.join(output_path, f'hybrid_stereo_{current_time}.log')),  # File output
        ],
    )
    ...
    input_files_path = find_all_files(os.path.join(parameters.get('input_path'), parameters.get('data_foldername')))

    # Process images to calculate the average for each 'zf' directory
    zf_directories = sorted(set([os.path.basename(os.path.dirname(path)) for path in input_files_path if os.path.basename(os.path.dirname(path)).startswith('zf')]))

    average_images_paths = []
    for zf_dir in zf_directories:
        # Filter relevant files in the directory
        filtered_files = sorted([file for file in input_files_path if f'{zf_dir}' in file and 'sVal.png' in file])
    ...
    light_directories = sorted(set([os.path.basename(os.path.dirname(path)) for path in input_files_path if os.path.basename(os.path.dirname(path)).startswith('L')]))

    for light_dir in light_directories:
        logging.info(f"... Processing light directory: {light_dir} ...")

        # Filter relevant files in the directory
        filtered_files = sorted([file for file in input_files_path if f'{light_dir}/zf' in file and 'sVal.png' in file])

        # Update parameters for the current directory
        parameters['filtered_dir'] = filtered_files
        parameters['output_path_multifocus'] = os.path.join(output_path, 'multifocus_stereo', light_dir)
    ...
    parameters['sMos_path_list'] = natsorted([file for file in output_files if 'sMos.png' in file and 'av' not in file])
