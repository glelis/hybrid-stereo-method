#define PROG_NAME "gus_integrate_recursive"
#define PROG_DESC "version of {gus_integrate_recursive} ({libpstzin0}) for G. Lelis"
...
#define PROG_INFO \
  ...
  "  2025-04-03 Converted to standalone package for G. Lelis.\n" \
  "\n" \
  "WARRANTY\n" \
  argparser_help_info_NO_WARRANTY "\n" \
  "\n" \
  "RIGHTS\n" \
  "  " gus_integrate_recursive_C_COPYRIGHT ".\n" \
  "\n" \
...
void tire_compute_and_write_height_map
  ...
  /* Computes the height map {Z} from the gradient map {G} and optional
    ...
    guess of the height at grid vertex {(X,Y)} for the iterative solver;
...
void tire_compute_and_write_height_map
  ...
  )
  {
    ...
    int32_t level = 0;
    bool_t extrapolate = FALSE; /* ??? Should be parameter? ??? */
    pst_integrate_recursive
      ( level, G, H, o->hints_weight, extrapolate, Z, R,
