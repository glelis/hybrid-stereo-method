...
- **Mitigado (b2c1eeb, 2026-06-06):** convenção end-to-end documentada no docstring de `reconcile_lights` (`main_wps.py`): image rows = y DOWN, normais n = (-dz/dx, -dz/dy, 1)/|.|, z cresce em direção à câmera (CONV-1). A decisão física (sinal `zMos` hints real vs `Z`) continua exigindo dado real com ground-truth — procedimento documentado no config via `flip_lights_y`.
...
- **Mitigado (b2c1eeb, 2026-06-06):** reconciliação explícita via `flip_lights_y`; decisão física continua exigindo dado real (procedimento documentado no config). `reconcile_lights(light_sources, flip_y)` em `main_wps.py`: quando `photometric.flip_lights_y: true`, nega a coluna y das luzes (y-up → y-down). Teste: `test_reconcile_lights_flip_y` (`tests/test_photometric_synthetic.py`): flip, identidade e não-mutação verificados; 12 passed.
...
|---|---|---|---|---|
| 1 | Eixo z / profundidade (direção + unidade) | `z_foc` → `zMos` (`mosaic.py:57,72`) → hints C → `height_map.npy` | **unidade: inconsistente (INT-04/CONV-4)**; **sinal-integrador: REFUTADO**; **sinal-físico: em aberto**; **convenção documentada: mitigado (b2c1eeb)** | `test_ramp_normals_decide_convention` PASSED (vencedor `+ax·x+ay·y`, RMSE 5.2e-5 vs 1.025). Task 12-ext: `a=+1.003`, `r=0.997` (frame sintético). Metade física requer dados reais. Convenção end-to-end fixada em `reconcile_lights` docstring. |
| 2 | Normais e luzes (frame / y-up vs y-down) | `lights.npy` POV-Ray → `reconcile_lights` → wps → normal→slope C (`pst_basic.c:49-50`) | **eixo-y integrador: REFUTADO**; **flip explícito: mitigado (b2c1eeb)**; **frame real: em aberto** | flip-y `+ax·x-ay·y` rejeitado (RMSE 0.381 vs 5.2e-5). `flip_lights_y: false` em ambos os configs; `reconcile_lights` expõe o flip como chave explícita testável. Frame POV-Ray real requer `lights.npy` real + gt de altura. |
